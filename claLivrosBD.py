...
class LivrosBD:
    ...
    def inserir(self, titulo: object, autor: object, editora: object, edicao: object, ano: object, local: object, data_compra: object) -> object:
        consulta = 'INSERT OR IGNORE INTO livros (titulo, autor, editora, edicao, ano, local, data_compra) VALUES (?, ?, ?, ?, ?, ?, ? )'
        self.cursor.execute(consulta, (titulo, autor, editora, edicao, ano, local, data_compra))
        self.conn.commit()

    def editar(self, titulo, autor, editora, edicao, ano, local, data_compra, id):
        consulta = 'UPDATE OR IGNORE livros SET titulo=?, autor=?, editora=?, edicao=?, ano=?, local=?, data_compra=? WHERE id=?'
        self.cursor.execute(consulta, (titulo, autor, editora, edicao, ano, local, data_compra, id))
        self.conn.commit()
